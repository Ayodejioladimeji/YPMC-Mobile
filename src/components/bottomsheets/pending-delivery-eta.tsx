...
const PendingDeliveryETA = forwardRef<BottomSheetModal, BottomSheetProps>(
    ({ closeModal }, ref) => {
        const headerHeight = useHeaderHeight();
        const [loading, setLoading] = useState(false)
        const { state, dispatch } = useContext(DataContext)
        const { shipping, shippingId, shippingType: type } = state
        ...
        const mainHeight = type === "multi" ? height * 0.4 : pHeight
        ...
        const handleContinue = async () => {
            ...
            let response;
            if (type === "basic") {
                response = await PostRequest(`/shipping/${shippingId}/assign-support-rider`, payload, state?.token)
                ...
                response = await PostRequest(`/shipping/${shippingId}/assign-support-rider-multiple`, payload, state?.token)
                if (response?.status === 200 || response.status === 201) {
                    dispatch({ type: ACTIONS.PROPOSED_RIDER, payload: response?.data?.data?.proposedRider })
                    dispatch({ type: ACTIONS.SHIPPING, payload: response?.data?.data })
                    router.push("/(app)/(tabs)/ship/rider-request")
        ...
        const handleRoute = () => {
            router.push("/(app)/(tabs)/track/schedule-selection")
            closeModal()
        }
        ...
        return (
            ...
            >
                <BottomSheetView style={styles.container}>
                    <Text style={styles.heading}>Select Delivery Preference</Text>
                            ...
                            >
                                <View style={styles.left}>
                                    ...
                                    <View>
                                        ...
                                        {/* <Text style={styles.eta}>
                                            <Text style={styles.etaLabel}>ETA:</Text> {moment(shipping?.estimatedPickupTime).format("lll")}
                                        </Text> */}
                                    </View>
                                </View>
...
export default PendingDeliveryETA;
