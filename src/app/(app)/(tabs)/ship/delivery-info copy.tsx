...
import SummarySheet from "@/components/ship/summary-sheet";
...
export default function DeliveryInfoForm() {
  ...
  const currentItem = useShippingStore((state) => state.currentItem);
    ...
    form.trigger().then((isValid) => {
      if (isValid) {
        if (state?.shippingType === 'basic') {
          ...
          dispatch({ type: ACTIONS.MULTIPLE_DATA, payload: payload })
